...
def evaluate(individual):
    ...
    # f = -tanh(365/(10**7.7) * (max(p,0)**0.15) * (100 - max(e,0)) * (t - 1500)) * 50 + (50*((p**0.15)/2)*((100-e)/100))
    if isnan(f):
        f = 0
    return f,p,e,t
...
if __name__ == "__main__":
    ...
    for generation in tqdm(range(ngens)):
        ...
        # print("Player Avg:", np.mean([ind[1] for ind in dataPerGen[generation+1]]))
        # print("Player Min:", np.min([ind[1] for ind in dataPerGen[generation+1]]))
        # print("Player Max:", np.max([ind[1] for ind in dataPerGen[generation+1]]))
        # print("Enemy Avg:", np.mean([ind[2] for ind in dataPerGen[generation+1]]))
        # print("Enemy Min:", np.min([ind[2] for ind in dataPerGen[generation+1]]))
        # print("Enemy Max:", np.max([ind[2] for ind in dataPerGen[generation+1]]))
        # print("Time Avg:", np.mean([ind[3] for ind in dataPerGen[generation+1]]))
        # print("Time Min:", np.min([ind[3] for ind in dataPerGen[generation+1]]))
        # print("Time Max:", np.max([ind[3] for ind in dataPerGen[generation+1]]))
